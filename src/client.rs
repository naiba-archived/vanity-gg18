...
use curv::{
    arithmetic::{traits::Converter, Zero},
...
fn main() {
    ...
    // find the key
    let u_bn = env::args().nth(3).unwrap();

    if u_bn == "create" {
        let mut d: Scalar<Secp256k1>;
        let mut expect_y: Point<Secp256k1>;
        loop {
            d = Scalar::random();
            expect_y = Point::generator() * &d;
            let mut hasher = Keccak256::new();
            let public_key = expect_y.to_bytes(false);
            hasher.update(&public_key[1..]);
            let out = hasher.finalize().to_vec();
            let addr = hex::encode(&out[12..]);
            if addr.starts_with("bb") {
                break;
            }
        }
        let u_vec: Vec<BigInt> = split_scalar(&d.to_bigint(), PARTIES);
        u_vec.iter().enumerate().for_each(|(index, u_i)| {
            println!("u_{}: {}", index + 1, u_i.to_hex());
        });
        println!("y: {:?}", expect_y);
        return;
    }
    ...
    let party_keys = Keys::create_from(
        Scalar::from_bytes(hex::decode(&u_bn).unwrap().as_slice()).unwrap(),
        party_num_int,
    );
    ...
    .unwrap();
    println!("y_sum: {:?}", y_sum_clone);
    fs::write(env::args().nth(2).unwrap(), keygen_json).expect("Unable to save !");
}

fn split_scalar(d: &BigInt, parties: u16) -> Vec<BigInt> {
    let mut splited_vec = Vec::new();
    for _ in 0..(parties - 1) {
        let r: Scalar<Secp256k1> = Scalar::random();
        splited_vec.push(r.to_bigint());
    }
    let sum = splited_vec.iter().fold(BigInt::zero(), |acc, x| acc + x);
    // d: 7
    // r1: 6, r2: 8
    // n: 10
    // r3: d - (6+8)%10 = 7 - 4 = 3
